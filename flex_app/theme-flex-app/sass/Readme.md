...
folder are mapped to the application's root namespace, 'Flexdata_theme_app'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "Flexdata_theme_app"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'Flexdata_theme_app' in the same way
as `"Flexdata_theme_app/sass/src"`.
